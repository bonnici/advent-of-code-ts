...
class Day5Solver extends Solver {
	...
	public init(inputFile: string): void {
		...
		this.sampleLog(`input ${this.input}, ${inputFile}`);
	}

	protected solvePart1(): string {
		this.sampleLog(`starting polymer ${this.input}`);
	...
	protected solvePart2(): string {
		...
		for (let unitType = 'a'; unitType <= 'z'; unitType = String.fromCharCode(unitType.charCodeAt(0) + 1)) {
			...
			this.sampleLog(`Starting polymer after removing ${unitType}: ${removed}`);
			const reacted = this.reactPolymer(removed);
			this.sampleLog(`Reacted length after removing ${unitType}: ${reacted.length} (${reacted})`);
	...
	private reactPolymer(polymer: string): string {
		...
		do {
			...
			for (let curUnit = 'a'; curUnit <= 'z'; curUnit = String.fromCharCode(curUnit.charCodeAt(0) + 1)) {
				...
				// this.sampleLog(`curUnit: ${curUnit}, curRegex: ${curRegex}, polymer after replacing: ${reacted}`);
			}
